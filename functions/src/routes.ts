...
// get by id
storyRoutes.get("/story/:id", (req, res) => {
...
// add a snippet
storyRoutes.post("/story", (req, res) => {
...
// update by id
storyRoutes.put("/story/:id", (req, res) => {
    const id = req.params.id;
    const snippet = req.body as Snippet;
    delete snippet._id;
    getClient().then(client => {
      return client.db().collection<Snippet>('snippets').replaceOne({ _id: new ObjectId(id) }, snippet).then(result => {
...
// delete by id
storyRoutes.delete("/story/:id", (req, res) => {
    const id = req.params.id;
    getClient().then(client => {
